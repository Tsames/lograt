...
class WorkoutRepository {
  ...
  Future<List<Workout>> getWorkoutSummariesAfterTime(int dateTimeThresholdInMilliseconds) async {
    try {
      final workoutModels = await _workoutDao.getWorkoutSummariesAfterTime(dateTimeThresholdInMilliseconds);
  ...
  Future<List<Workout>> getWorkoutSummaries({int? limit, int? offset}) async {
    try {
      final workoutModels = await _workoutDao.getWorkoutSummaries(limit: limit, offset: offset);
  ...
  Future<Exercise?> _buildExerciseEntity(ExerciseModel exerciseModel) async {
    try {
      ...
      final exerciseSetEntities = exerciseSetModels.map((set) => set.toEntity()).toList();
      return exerciseModel.toEntity(exerciseType: exerciseTypeModel.toEntity(), sets: exerciseSetEntities);
    } catch (e) {
      return null;
    }
  }
  ...
  Future<List<ExerciseType>> getExerciseTypes({int? limit, int? offset, Transaction? txn}) async {
    final exerciseTypeModels = await _exerciseTypeDao.getAll(limit: limit, offset: offset, txn: txn);
    ...
    try {
      ...
      return await db.transaction<void>((txn) async {
        for (final workout in workouts) {
          ...
          final workoutId = await _workoutDao.insertWithTransaction(WorkoutModel.fromEntity(workout), txn);
          ...
          for (final exercise in workout.exercises) {
            ...
            if (exercise.exerciseType.id != null) {
              ...
              final existingExerciseTypeByName = await _exerciseTypeDao.getByName(exercise.exerciseType.name, txn);
