...
class Exercise {
  ...
  Exercise copyWithNewSet() {
    if (sets.isEmpty) return copyWith(sets: [ExerciseSet(order: 0, setType: SetType.warmup)]);
    final lastSet = sets.last;
    return copyWith(
      sets: [
        ...sets,
        ExerciseSet(order: lastSet.order + 1, setType: lastSet.setType, units: lastSet.units),
      ],
    );
  }
