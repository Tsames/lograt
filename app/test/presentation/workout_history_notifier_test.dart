...
void main() {
  ...
  group('Workout History Notifier Tests', () {
    ...
    setUp(() async {
      ...
      // final muscleGroupToWorkoutTemplateDao = MuscleGroupToWorkoutTemplateDao(testDatabase);
      // final muscleGroupToExerciseTypeDao = MuscleGroupToExerciseTypeDao(testDatabase);
      //
      ...
      // );
      //
      // getPaginatedSortedWorkoutsUsecase = GetPaginatedSortedWorkoutsUsecase(testRepository, pageSize: 10);
    ...
    //   test('workouts older than 90 days should not be saved to notifier state', () async {
    //     final testWorkouts = [
    ...
    //     expect(
    //       testNotifier.state.workouts.whereType<Workout>().length,
    //       testWorkouts.where((workout) => workout.date.isAfter(now.beginningOfTheLastThreeMonths)).length,
    //     );
    //   });
    //
    //   test('notifier state should not include this week section header when no workouts happened in that time', () async {
    ...
    //     await testNotifier.loadPaginatedWorkouts();
    //
    //     final headers = testNotifier.state.workouts.whereType<WorkoutHistorySectionHeader>();
    //     expect(headers.whereType<ThisWeekWorkoutHistorySectionHeader>(), isEmpty);
    //     expect(testNotifier.state.hasWorkoutsThisWeekSectionHeader, false);
    //
    //     expect(headers.whereType<InTheLastMonthWorkoutHistorySectionHeader>(), isNotEmpty);
    //     expect(testNotifier.state.hasWorkoutsInLastMonthSectionHeader, true);
    //
    //     expect(headers.whereType<InTheLastThreeMonthsWorkoutHistorySectionHeader>(), isNotEmpty);
    //     expect(testNotifier.state.hasWorkoutsInLastThreeMonthsSectionHeader, true);
    //   });
    //
    //   test(
    ...
    //
    //       final headers = testNotifier.state.workouts.whereType<WorkoutHistorySectionHeader>();
    //       expect(headers.whereType<ThisWeekWorkoutHistorySectionHeader>(), isNotEmpty);
    //       expect(testNotifier.state.hasWorkoutsThisWeekSectionHeader, true);
    //
    //       expect(headers.whereType<InTheLastMonthWorkoutHistorySectionHeader>(), isEmpty);
    ...
    //       expect(testNotifier.state.hasWorkoutsInLastMonthSectionHeader, true);
    //
    //       expect(headers.whereType<InTheLastThreeMonthsWorkoutHistorySectionHeader>(), isEmpty);
    //       expect(testNotifier.state.hasWorkoutsInLastThreeMonthsSectionHeader, false);
    //     },
    //   );
    //
    //   test('notifier state should be empty if only workouts older than 90 days exist in db', () async {
    //     await testRepository.seedWorkoutData([
    ...
    //
    //     await testNotifier.loadPaginatedWorkouts();
    //     expect(testNotifier.state.workouts, isEmpty);
    //   });
    //
    ...
    //   });
    //
    //   test('notifier state should not have a weekly headers after this week section header', () async {
    //     await testRepository.seedWorkoutData([
    ...
    //     await testNotifier.loadPaginatedWorkouts();
    //
    //     final weekHeaders = testNotifier.state.workouts.whereType<WeekWorkoutHistorySectionHeader>();
    ...
    //       await testNotifier.loadPaginatedWorkouts();
    //
    //       expect(testNotifier.state.workouts.length, 9);
    //
    //       expect(testNotifier.state.workouts.whereType<WeekWorkoutHistorySectionHeader>().length, 3);
    //       expect(testNotifier.state.workouts[0], isA<InTheLastMonthWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[1], isA<WeekWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[2], isA<Workout>());
    //       expect(testNotifier.state.workouts[3], isA<Workout>());
    //       expect(testNotifier.state.workouts[4], isA<WeekWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[5], isA<Workout>());
    //       expect(testNotifier.state.workouts[6], isA<Workout>());
    //       expect(testNotifier.state.workouts[7], isA<WeekWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[8], isA<Workout>());
    //     },
    //   );
    //
    //   test(
    ...
    //       await testNotifier.loadPaginatedWorkouts();
    //
    //       expect(testNotifier.state.workouts.length, 5);
    //
    //       expect(testNotifier.state.workouts.whereType<WeekWorkoutHistorySectionHeader>().length, 2);
    //       expect(testNotifier.state.workouts[0], isA<InTheLastThreeMonthsWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[1], isA<WeekWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[2], isA<Workout>());
    //       expect(testNotifier.state.workouts[3], isA<WeekWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[4], isA<Workout>());
    //     },
    //   );
    //
    //   test(
    ...
    //       await testNotifier.loadPaginatedWorkouts();
    //
    //       expect(testNotifier.state.workouts.length, 18);
    //
    //       expect(testNotifier.state.workouts.whereType<Workout>().length, 10);
    //       expect(testNotifier.state.workouts.whereType<ThisWeekWorkoutHistorySectionHeader>().length, 1);
    //       expect(testNotifier.state.workouts.whereType<InTheLastMonthWorkoutHistorySectionHeader>().length, 1);
    //       expect(testNotifier.state.workouts.whereType<InTheLastThreeMonthsWorkoutHistorySectionHeader>().length, 1);
    //       expect(testNotifier.state.workouts.whereType<WeekWorkoutHistorySectionHeader>().length, 5);
    //
    //       expect(testNotifier.state.workouts[0], isA<ThisWeekWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[1], isA<Workout>());
    //       expect(testNotifier.state.workouts[2], isA<Workout>());
    //       expect(testNotifier.state.workouts[3], isA<Workout>());
    //       expect(testNotifier.state.workouts[4], isA<Workout>());
    //       expect(testNotifier.state.workouts[5], isA<InTheLastMonthWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[6], isA<WeekWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[7], isA<Workout>());
    //       expect(testNotifier.state.workouts[8], isA<Workout>());
    //       expect(testNotifier.state.workouts[9], isA<WeekWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[10], isA<Workout>());
    //       expect(testNotifier.state.workouts[11], isA<InTheLastThreeMonthsWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[12], isA<WeekWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[13], isA<Workout>());
    //       expect(testNotifier.state.workouts[14], isA<WeekWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[15], isA<Workout>());
    //       expect(testNotifier.state.workouts[16], isA<WeekWorkoutHistorySectionHeader>());
    //       expect(testNotifier.state.workouts[17], isA<Workout>());
    //     },
    //   );
    //
    //   test('notifier state should correctly interleave week section header when pagination ends mid week', () async {
    //     final testWorkouts = [
    //       Workout(date: now.beginningOfTheWeek.add(Duration(days: 1))),
    //       Workout(date: now.beginningOfTheWeek.add(Duration(days: 2))),
    ...
    //     // First Page
    //     await testNotifier.loadPaginatedWorkouts();
    //     expect(testNotifier.state.workouts.length, 13);
    //     expect(testNotifier.state.workouts.whereType<Workout>().length, 10);
    //     expect(testNotifier.state.workouts.whereType<ThisWeekWorkoutHistorySectionHeader>().length, 1);
    //     expect(testNotifier.state.workouts.whereType<InTheLastMonthWorkoutHistorySectionHeader>().length, 1);
    //     expect(testNotifier.state.workouts.whereType<InTheLastThreeMonthsWorkoutHistorySectionHeader>(), isEmpty);
    //     expect(testNotifier.state.workouts.whereType<WeekWorkoutHistorySectionHeader>().length, 1);
    //
    //     expect(testNotifier.state.workouts[0], isA<ThisWeekWorkoutHistorySectionHeader>());
    //     expect(testNotifier.state.workouts[1], isA<Workout>());
    //     expect(testNotifier.state.workouts[2], isA<Workout>());
    ...
    //     expect(testNotifier.state.workouts[5], isA<Workout>());
    //     expect(testNotifier.state.workouts[6], isA<Workout>());
    //     expect(testNotifier.state.workouts[7], isA<InTheLastMonthWorkoutHistorySectionHeader>());
    //     expect(testNotifier.state.workouts[8], isA<WeekWorkoutHistorySectionHeader>());
    //     expect(testNotifier.state.workouts[9], isA<Workout>());
    //     expect(testNotifier.state.workouts[10], isA<Workout>());
    //     expect(testNotifier.state.workouts[11], isA<Workout>());
    //     expect(testNotifier.state.workouts[12], isA<Workout>());
    //
    //     // Second Page
    //     await testNotifier.loadPaginatedWorkouts();
    //     expect(testNotifier.state.workouts.length, 19);
    //     expect(testNotifier.state.workouts[13], isA<Workout>());
    //     expect(testNotifier.state.workouts[14], isA<Workout>());
    //     expect(testNotifier.state.workouts[15], isA<Workout>());
    //     expect(testNotifier.state.workouts[16], isA<WeekWorkoutHistorySectionHeader>());
    //     expect(testNotifier.state.workouts[17], isA<Workout>());
    //     expect(testNotifier.state.workouts[18], isA<Workout>());
    //   });
  });
}
